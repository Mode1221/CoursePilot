...
_ASPECTS: list[tuple[str, str, str]] = [
    ...
    # "분위기는 좋은데" 처럼 조사가 끼는 경우가 흔해 사이 조사를 허용한다
    (
        "분위기",
        r"분위기\s*(?:가|는|도)?\s*(?:좋|최고|예쁘)|뷰\s*(?:가|는)?\s*(?:좋|맛집)"
        r"|인테리어\s*(?:가|는)?\s*(?:예쁘|좋)",
        r"분위기\s*(?:가|는)?\s*(?:별로|아쉽)",
    ),
    ("소음", r"조용|한적|차분", r"시끄|소란|웅성"),
    ("가성비", r"가성비|가격\s*(착|저렴)|합리적", r"비싸|가격\s*대비\s*아쉽|바가지"),
    ("친절", r"친절|응대\s*(?:가|는)?\s*(?:좋|훌륭)", r"불친절|응대\s*(?:가|는)?\s*(?:별로|나쁨)"),
    ("맛", r"맛(있|집)|존맛|훌륭", r"맛\s*(?:은|이|도)?\s*(?:없|별로)|싱겁|짜"),
    ("청결", r"깨끗|청결|위생\s*(?:이|은)?\s*좋", r"더럽|지저분|위생\s*(?:이|은)?\s*(?:별로|안)"),
    # "주차장 넓어요"가 좌석으로 잡히던 오탐 → 자리·공간을 가리키는 표현만 본다
    (
        "좌석",
        r"자리\s*(?:가|는)?\s*(?:넉넉|많|넓)|넓은\s*(?:공간|매장|홀|내부)|테이블\s*(?:넉넉|많)",
        r"자리\s*(?:가|는)?\s*(?:없|부족|좁)|매장\s*(?:이|은)?\s*좁|협소",
    ),
    # 실사용에서 자주 갈리는 축 보강
