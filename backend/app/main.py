...
async def generate(
    ...
    async def action() -> GenerateResponse:
        ...
        if edit_cmd.action == "none" and course.items and _is_question(req.text):
            ...
            course.locked = False
            await broadcast_lock(course_id, False)
            ai_text = _course_answer(course, req.text)
            chat_store.append(course_id, "ai", ai_text)
            await broadcast_message(course_id, "ai", ai_text)
...
def _fact_answer(course: Course, text: str) -> str | None:
...
def _course_answer(course: Course, text: str = "") -> str:
    ...
    if any(w in text for w in _COST_WORDS):
        return _cost_answer(course)
    return _course_answer_summary(course)


def _course_answer_summary(course: Course) -> str:
