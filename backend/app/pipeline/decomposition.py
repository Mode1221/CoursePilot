...
# "비싼 곳 말고" 처럼 사이에 '곳/데/장소'가 끼면 그 앞 단어가 진짜 제외 대상이다.
_EXCLUDE_RE = re.compile(
    r"([가-힣]{1,6}?)\s*(?:곳|데|장소)?\s*(?:은|는|을|를|이|가|거|건)?\s*"
    r"(?:빼고|제외하고|제외|말고|없이)"
)
...
_COMPANION_MAP = {
    ...
    # 실사용 표현: 회사 동료 모임은 "회식"이라고 부르지 않는 경우가 더 많다
    "회식": [
        "회식", "단체", "팀", "동료", "술자리", "부서", "회사 사람", "직장 동료",
        "팀원", "상사", "임원", "송년회", "신년회", "환영회", "워크샵", "워크숍",
    ],
    "가족": [
...
SYNONYMS: dict[str, str] = {
    # "술 한잔", "술만" — '술' 한 글자로는 "예술"에 걸리므로 앞뒤를 붙여서 본다
    "술 한잔": "술집",
    "술한잔": "술집",
    "술만": "술집",
    "한잔하": "술집",
