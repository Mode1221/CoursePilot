"""Google Places SKU: 영업시간·평점은 같은 Enterprise 콜로 받고 TTL 안에서 재사용한다."""
...
class _FakeClient(GooglePlacesClient):
    def __init__(self, hours=HOURS_PAYLOAD, rating=(4.3, 120)):
        ...
        if rating is None:
            self._hours.pop("rating", None)
            self._hours["userRatingCount"] = 3  # 표본 부족
        self.calls: list[str] = []
    ...
    async def fetch_details(self, place_id):
        self.calls.append("details")
        return self._hours


def test_상세_필드마스크가_한_콜에_모여_있다():
    """영업시간·평점이 같은 Enterprise SKU 라 나눠 부르면 한도만 두 배로 쓴다."""
    assert IDS_ONLY_MASK == "places.id"
    for field in ("regularOpeningHours", "currentOpeningHours", "businessStatus",
...
async def test_영업시간_갱신은_매핑_1콜_상세_1콜():
    client = _FakeClient()
    place = await client.refresh_details(_place(), weekday=0)
    assert client.calls == ["map", "details"]
...
async def test_place_id를_알면_매핑을_건너뛴다():
    client = _FakeClient()
    await client.refresh_details(_place(google_place_id="gp-1"), weekday=0)
    assert client.calls == ["details"]


async def test_TTL_안이면_호출하지_않는다():
    client = _FakeClient()
    fresh = _place(
        hours_checked_at=datetime.now(UTC) - timedelta(days=1),
        rating_checked_at=datetime.now(UTC) - timedelta(days=1),
    )
    await client.refresh_details(fresh)
    assert client.calls == []
...
async def test_호출_실패는_코스를_깨지_않고_확인_필요로_남는다():
    ...
    place = await _Broken().refresh_details(_place(), weekday=0)
    assert place.hours_unverified is True
...
async def test_평가수가_적으면_평점을_쓰지_않는다():
    ...
    place = await client.refresh_details(_place())
    assert place.rating is None
    assert place.rating_checked_at is not None  # 다시 묻지 않도록 시각은 남긴다
    assert MIN_RATING_COUNT == 30


async def test_한_콜로_영업시간과_평점을_함께_받는다():
    client = _FakeClient()
    place = await client.refresh_details(_place(google_place_id="gp-1"), weekday=0)
    assert client.calls == ["details"]  # 평점 때문에 추가 콜이 나가지 않는다
...
async def test_확정_코스만_갱신한다(monkeypatch):
    ...
    assert client.calls.count("details") == 1
