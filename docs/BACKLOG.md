...
8. **행정** (코드 밖) — 포트원 가맹계약, SMS 발신번호 등록, 개인정보처리방침(삭제 요청은 회원 탈퇴 API 로 처리 가능).
   약관·처리방침 페이지는 초안 상태로 배포돼 있다(`/terms`, `/privacy`) — 남은 일: 변호사 검토,
   개인정보 보호책임자 지정, 상단 "법률 검토 전 초안" 배너 제거.

## 확인 대기
- **GHCR 이미지 발행** — Release images 워크플로가 초록으로 끝나고
  `ghcr.io/mode1221/coursepilot-{backend,frontend}:latest` 가 실제로 생겼는지,
  패키지 공개 설정이 public 인지 확인해야 완료다(레포가 public 이라 무료 pull).

## 최근 처리 (2026-09-10, 7차 — 시드 데이터)
- 시드: 상권 24곳 3,600여 건(슬롯·영업시간·가격·평점 분포를 실제와 비슷하게), is_mock 으로 일괄 삭제
- 어댑터: 키 없고 시드가 있으면 검색이 시드를 쓴다 → 키 없이 코스 생성·편집·공유 확인 가능
- 상권: 좌표 3곳 수정(망원·청담·삼성), 붙어 있는 상권 반경 축소(겹침 21→15쌍, 최대 1503→703m)
- 도구: districts_map.py 배치 HTML + 겹침 리포트

## 최근 처리 (2026-09-10, 6차 — 운영 스크립트)
- 백업: pg_dump gzip 7일 보관, 단계별 실패 감지 후 웹훅 알림
- 크론: 배치 3종 + 백업 + 디스크 점검을 deploy.sh 가 설치(기존 항목 보존)
- 디스크: 사용률 85% 초과 시 알림
- 문서: 첫 기동 체크리스트(헬스체크 → 스모크 → 대장 → 백업 수동 1회 → 크론 확인)

## 최근 처리 (2026-09-10, 5차 — 프로덕션 안전장치)
- 기동 거부: production 필수 설정(SESSION_SECRET·ADMIN_TOKEN·DB 비밀번호) 없으면 부팅 실패
- rate limit: 신뢰 프록시 기반 X-Forwarded-For 처리(위조 우회·전체 차단 동시 해결)
- 인증: 운영에서 SMS 키 없으면 503(개발 폴백 코드 노출 차단)
- 로그: 민감값 마스킹 헬퍼 + 실제 요청 로그를 훑는 회귀 테스트
- compose: 전 서비스 헬스체크 + 로그 로테이션(10MB×3), /health 에 DB·연동 상태
