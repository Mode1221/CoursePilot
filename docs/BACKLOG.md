...
7. **관측성 강화** (🟡) — 구조적 로깅 + 인메모리 메트릭(`GET /admin/metrics`, `app/metrics.py`)에 외부 연동 폴백 비율·임계 알림(`alerts`) 포함. 임계 진입·회복 시 경고 로그. 남은 일: 외부 에러 트래킹(Sentry 등) 연동, 다중 인스턴스 집계, 알림 외부 발송(웹훅/메신저).
